...
class SimplifiedDATBlock(nn.Module):
    def __init__(self, dim, nh, ws, ffn_exp, aim_re, btype, dp, qkv_b=False):
        ...
        self.attn = FastSpatialWindowAttention(dim, ws, nh, qkv_b) if btype == 'spatial' else FastChannelAttention(dim, nh, qkv_b)
...
class SimplifiedResidualGroup(nn.Module):
    def __init__(self, dim, depth, nh, ws, ffn_exp, aim_re, pattern, dp_rates):
        super().__init__()
        self.blocks = nn.ModuleList([SimplifiedDATBlock(dim, nh, ws, ffn_exp, aim_re, pattern[i % len(pattern)], dp_rates[i]) for i in range(depth)])
...
class lhan(nn.Module):
    def __init__(self, num_in_ch=3, num_out_ch=3, upscaling_factor=4, embed_dim=120, num_groups=4, depth_per_group=3, num_heads=4, window_size=8, ffn_expansion_ratio=2.0, aim_reduction_ratio=8, group_block_pattern=['spatial', 'channel'], drop_path_rate=0.1, upsampler_type='pixelshuffle', img_range=1.0, **kwargs):
        super().__init__()
        ...
        self.groups = nn.ModuleList([
            SimplifiedResidualGroup(embed_dim, ad, num_heads, window_size, ffn_expansion_ratio, aim_reduction_ratio, group_block_pattern, dpr[i * ad:(i + 1) * ad]) for i in range(num_groups)
        ])
...
def lhan_tiny(**kwargs):
    """A tiny version of the LHAN model."""
    return lhan(
        embed_dim=96, num_groups=2, depth_per_group=2, num_heads=3,
        ffn_expansion_ratio=1.5, drop_path_rate=0.05,
        upsampler_type='pixelshuffle', **kwargs
    )

def lhan_light(**kwargs):
    """A light version of the LHAN model."""
    return lhan(
        embed_dim=108, num_groups=3, depth_per_group=2, num_heads=4,
        ffn_expansion_ratio=2.0, drop_path_rate=0.08,
        upsampler_type='nearest_conv', **kwargs
    )

def lhan_medium(**kwargs):
    """A medium (base) version of the LHAN model."""
    return lhan(
        embed_dim=120, num_groups=4, depth_per_group=3, num_heads=4,
        ffn_expansion_ratio=2.0, drop_path_rate=0.1,
        upsampler_type='transpose_conv', **kwargs
    )
...
if __name__ == '__main__':
    ...
    # Instantiate the model (you can choose lhan, lhan_tiny, lhan_light, or lhan_medium)
    # The default upsampler for lhan_tiny is 'pixelshuffle'
    model = lhan_tiny(upscaling_factor=scale)
    
    # Count the number of parameters
    num_params = sum(p.numel() for p in model.parameters())
    print(f"Model: lhan_tiny")
    print(f"Number of parameters: {num_params / 1e6:.2f}M")
    
    # Create a dummy input tensor
    # Shape: (batch_size, num_channels, height, width)
    dummy_input = torch.rand(1, 3, input_resolution, input_resolution)
    ...
    # Print the shapes to verify the upscaling
    print(f"Input shape:  {dummy_input.shape}")
    print(f"Output shape: {output.shape}")
    
    # --- Test another variant ---
    model_medium = lhan_medium(upscaling_factor=scale)
    num_params_medium = sum(p.numel() for p in model_medium.parameters())
    print(f"\nModel: lhan_medium")
    print(f"Number of parameters: {num_params_medium / 1e6:.2f}M")
